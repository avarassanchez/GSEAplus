...
```{r GSEA_execution, include=FALSE}
count = 1
null_dist <- c()
...
for(data_frame in params$gene_list){ # Traverse all the input gene lists
  for(list in 1:length(params$candidate_list)){ # Traverse all the candidate gene lists
    ...
    GSEA_plot <- ggplot() +
            ...
            labs(x = "Rank in Ordered Dataset", y = "Running Enrichment Score", caption = count, color = "Comparison Number") 
    ...
    null_list <- list(null_list, NULL_plot)
  }
}
```
